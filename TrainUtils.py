...
def CalcLoss(env, q, hnet, adj_net):
    q0 = env.sample_q(1)
    p0 = adj_net(torch.tensor(q0, dtype = torch.float32))
    pT = adj_net(torch.tensor(q, dtype = torch.float32))
...
def sample_step(q, p, env, HDnet, times, memory, control_coef, device):
    ...
    for i in range(qps.shape[0]):
        ...
        u = (1.0 / (2 * control_coef)) * np.einsum('ijk,ij->ik', env.f_u(qi_np)[None, :], pi_np) * -1
        #print(qi_np)
        #print(u)
        #print('-----------------------------------------------------')
...
def train_hnet(sigma, device, env, num_episodes, memory, adj_net, hnet, hnet_target,
    T_end=5.0, n_timesteps=100, control_coef=0.5, use_adj_net=False,
    update_interval=2, rate=1, batch_size_sample=256, batch_size=32,
...
def fit_adjoint(q, times, adj_net, HDnet, optim_adj, device, env):
    total_loss = 0
    for i in range(64):
        zero_tensor = torch.zeros(13, dtype=torch.float, device=device)
        criterion = torch.nn.SmoothL1Loss()
        ...
        loss = CalcLoss(env, q[i], HDNet, adj_net)
...
def train_adjoint(sigma, device, env, num_episodes, adj_net, hnet,
    ...
    while iter < num_adj_train_max:
        loss_adj = fit_adjoint(next(generator), times, adj_net, HDnet, optim_adj, device, env)
        total_loss += loss_adj
        if iter % 3 == 0:
...
def training(sigma, device, env, env_name,
    ...
    T_hnet=5.0, T_adj=2.0, n_timesteps=100, control_coef=0.5,
    ...
    for i in range(num_warmup):
        ...
        u = -env.f_u(q).transpose() @ p.detach().numpy().reshape(1, p.shape[0]).transpose()
        print(u)
